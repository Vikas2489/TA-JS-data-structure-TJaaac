...
function greet1() {
    let characterName = "Arya";
    let title = "Lady of Winterfell";
    return alert(`I am ${characterName} and my title is ${title}`);
}

character.greet = greet1();
...
let agee = function age(age) {
    if (age > 18) {
        return true;
    } else { return false; }
}

character.isAdult = agee;
...
let changeIsAdult = function(flipAdult) {
    if (flipAdult === true) {
        return !flipAdult;
    } else if (flipAdult === false) {
        return !flipAdult;
    }
}
changeIsAdult(true);
